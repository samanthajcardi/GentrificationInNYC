...
with open ('BrooklynRent1999.csv', 'rU') as f:
  ...
  #  l=reader.next()
    ...
    p=Bar(d,values='values',label=CatAttr(sort=False, columns=['labels']), xlabel='Gross Rent', 
          ylabel='Number of People in Thousands', agg='min', title='Brooklyn Gross Rent 1999')



#This is if i wanted to mae it a linegraph     
#    p = figure(width=800, height=500)
#    category=['< $500', '> $500', '$1,000-$1,999', '$2,000 plus']
#    d=pd.DataFrame({'labels':category, 'values':yrange}, index=[1,2,3,4])
#    p.line(BlockList, line_width=2)
#    p.title='Brooklyn Rent'
#    p.xaxis.axis_label = 'Income'
#    p.yaxis.axis_label = 'Number of people in Thousands'
#    p.xaxis.x_range_name=


   # p.Line(BlockList, yrange, color='orange', legend='Brooklyn Rent')
    

    output_file("BrooklynRent1999.html", title="Brooklyn Rent 1999")
